...
def joinFactors(factors):
    ...
    for factor in factors:
        for uncond_var in factor.unconditionedVariables():
            unconditioned.add(uncond_var)
    for factor in factors:
        for cond_var in factor.conditionedVariables():
            if cond_var not in unconditioned:
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        for uncond_var in factor.unconditionedVariables():
            if uncond_var != eliminationVariable:
                uncond_vars.add(uncond_var)
        dict = factor.variableDomainsDict()
        domains = dict[eliminationVariable]
        new_factor = Factor(uncond_vars, cond_vars, dict)
...
def normalize(factor):
    ...
    for uncond_var in uncond_vars:
        if len(variableDomainsDict[uncond_var]) == 1:
            cond_vars.add(uncond_var)
    vars_uncond = set()
    for uncond_var in uncond_vars:
        if uncond_var not in cond_vars:
            vars_uncond.add(uncond_var)
    new_factor = Factor(vars_uncond, cond_vars, variableDomainsDict)

    for assignment in new_factor.getAllPossibleAssignmentDicts():
        norm = factor.getProbability(assignment) / sum
        new_factor.setProbability(assignment, norm)
    return new_factor
    # util.raiseNotDefined()
