...
def fillObsCPT(bayesNet, gameState):
    ...
    # util.raiseNotDefined()
def get_closest_house(gameState, position):
    dist = float("inf")
    closest = gameState.getPossibleHouses()[0]
    for house in gameState.getPossibleHouses():
        temp = util.manhattanDistance(position, house)
        if temp < dist:
            dist = temp
            closest = house
    return closest

def get_prob(pos, ghost_house, food_house, color):
    prob = 0
    if pos != ghost_house and pos != food_house:
        if color == RED_OBS_VAL or color == BLUE_OBS_VAL:
            prob = 0
        else:
            prob = 1
    elif ghost_house == food_house or pos == food_house:
        ...
        if color == RED_OBS_VAL:
            ...
            prob = 0
    return prob
def get_closest_pos(gameState, position):

    closest_house = get_closest_house(gameState, position)
    location = gameState.getPossibleHouses()

    if closest_house == location[0]:
        return BOTTOM_LEFT_VAL
    if closest_house == location[1]:
        return TOP_LEFT_VAL
    if closest_house == location[2]:
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        left_prob = 0
        right_prob = 0
        pdf = inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)
        for assignment in pdf.getAllPossibleAssignmentDicts():
            if assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                right_prob = pdf.getProbability(assignment)
            elif assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL:
                left_prob = pdf.getProbability(assignment)

        rightExpectedValue = right_prob * WON_GAME_REWARD + (1 - right_prob) * GHOST_COLLISION_REWARD
        leftExpectedValue = left_prob * WON_GAME_REWARD + (1 - left_prob) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, explorationEvidence in self.getExplorationProbsAndOutcomes(evidence):
            new_evidence_dict = evidence.copy()
            new_evidence_dict.update(explorationEvidence)
            tup = self.computeEnterValues(new_evidence_dict, enterEliminationOrder)
            expectedValue += max(tup[0], tup[1]) * prob
