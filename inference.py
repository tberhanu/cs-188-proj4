...
from factorOperations import joinFactorsByVariableWithCallTracking, joinFactors, joinFactorsByVariable, eliminate
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        if len(eliminationOrder) < 1:
            ...
            for var in eliminationOrder:
                currentFactorsNotToJoin, joinedFactor = joinFactorsByVariable(currentFactorsNotToJoin, var)
                if len(joinedFactor.unconditionedVariables()) != 1:
                    fact = eliminate(joinedFactor, var)
                    currentFactorsNotToJoin.append(fact)
            final_factor = joinFactors(currentFactorsNotToJoin)
